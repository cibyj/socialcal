...
function fnUrl(path) {
  // helper: point to /.netlify/functions/<name> or allow overriding
...
export default function App(){
  ...
  async function saveEmail(){
    await fetch(fnUrl('setReminderEmail'), {method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({reminder_email: email})});
    alert('Saved');
  }
...
async function addOrUpdate() {
  ...
  async function deleteEvent(id){
    ...
    await fetch(fnUrl('deleteEvent'), {method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({id})});
    load();
  }
